...
export interface TourMapMarker extends Point {
  id:string; name:string; kind:'center'|'facility'|'gym'|'person'|'pokemon'|'exit'; destination?:MapId; event?:string; npcId?:string;
}
...
export function tourMapMarkers(map:GameMap):TourMapMarker[]{
  ...
  // Every registered outdoor passage and dungeon staircase is a travel exit.
  // Building and floor doors are already represented above.
  const exitDirection:Record<string,string>={left:'←',right:'→',up:'↑',down:'↓'};
  const exits=map.warps.filter(w=>!doorTargets.has(w.to)).map(w=>({id:'exit:'+w.to,name:`${exitDirection[w.entry]??'◇'} ${getMap(w.to).name}`,kind:'exit' as const,x:w.x,y:w.y,destination:w.to}));
  ...
  return [
    ...
    ...exits,
    ...landmarks,
    ...map.npcs.map(n=>({id:n.id,npcId:n.id,name:n.name,kind:n.id==='tourPokemon'||n.sprite.startsWith('field-')?'pokemon' as const:'person' as const,x:n.x,y:n.y,destination:map.id,event:n.dialogue})),
  ];
}
export function tourMarkerBounds(map:GameMap,marker:Point){
  ...
  // Renderer draws an 8px marker regardless of map scale. Tall routes must
  // retain that full clickable footprint instead of shrinking it with tiles.
  const size=Math.max(8,Math.min(12,layout.scale*3));return {x:cx-size/2,y:cy-size/2,w:size,h:size,cx,cy};
}
