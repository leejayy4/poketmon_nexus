...
import { setupNexusProfile } from './nexus-profile';
const game=new Engine();
const updateProfile=setupNexusProfile(game);
...
window.addEventListener('keydown',event=>{if(event.target instanceof HTMLElement&&event.target.closest('#test-panel, .header-actions, #help, .nexus-profile'))return;const key=normalize(event.key);if(allowed(key)){event.preventDefault();game.press(key,event.repeat)}});
document.addEventListener('focusin',event=>{if(event.target instanceof HTMLElement&&event.target.closest('#test-panel, .header-actions, #help, .nexus-profile'))game.clearInput()});
