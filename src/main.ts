...
document.querySelector('#sound')!.addEventListener('click',()=>{const enabled=game.audio.toggle();const b=document.querySelector('#sound')!;b.textContent=enabled?'소리 ON':'소리 OFF';b.setAttribute('aria-label',enabled?'소리 끄기':'소리 켜기')});
