...
export function sinnohEvent(g:Engine,id:string):boolean {
  if(id==='sinnohGymGuide'){
    ...
    g.say('체육관 안내원',['이곳은 '+gym.name+'의 체육관이에요.\n앞선 배지를 얻었다면 도전하세요.','레벨 '+gym.level+'과 상처약을 준비하세요.\n센터에서 회복과 보충을 받아요.']);return true;
  }
  if(id==='trailGuide'){
    g.healParty();g.save.inventory.potions=Math.max(2,g.save.inventory.potions);g.persist();
    ...
    g.say('길 안내원',['포켓몬을 회복하고 상처약을\n2개까지 채웠어요.',...directions]);return true;
  }
  if(id==='sinnohGuide'){
    g.say('도시 안내원',g.save.map==='tour_eterna'?['천관산 연결길을 지나면 연고예요.\n먼저 유채의 체육관에 도전해 보세요.','얼마 전 시계가 3초 늦어졌어요.\n다른 도시에서도 같은 일이 있었대요.']:g.save.map==='tour_hearthome'?['이곳은 멜리사의 체육관이 있는 연고예요.\n길 안내판을 따라 장막으로 갈 수 있어요.','역의 시계 기록도 3초가 어긋났대요.\n장막 연구원이 기록을 모으고 있어요.']:['GYM 간판이 있는 곳이 자두의 체육관이에요.\n승리하면 마을의 관측 연구원을 만나세요.']);return true;
  }
  if(id==='observation'){
