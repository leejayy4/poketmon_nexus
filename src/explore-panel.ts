...
export function setupExplorePanel(game:Engine,root:HTMLElement){
  ...
  const panelPlaces=()=>[...new Map([...PLACES,...(['tour_celestic'] as TourId[]).map(placeById).filter((p):p is NonNullable<typeof p>=>Boolean(p))].map(p=>[p.id,p])).values()];
  let previousJournal='';
  function refreshJournal(){
    ...
    section.querySelector('#tour-visit-summary')!.textContent='야외 '+summary.outdoors+'/'+panelPlaces().length+' · 실내 '+summary.interiors+'/'+Object.keys(TOUR_INTERIORS).length+' 방문\n'+region+' '+summary.regionVisited+'/'+summary.regionTotal+' · ✓: 방문';
