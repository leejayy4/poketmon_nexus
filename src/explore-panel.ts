...
export function setupExplorePanel(game:Engine,root:HTMLElement){
  ...
  function renderRegion(){const places=PLACES.filter(p=>p.region===region);select.innerHTML=places.map(p=>`<option value="${p.id}">${p.name}</option>`).join('');const lines=places.flatMap(p=>TOUR_NEIGHBORS(p.id).filter(id=>id>p.id&&placeById(id)?.region===region).map(id=>{const q=placeById(id)!;return `<line x1="${14+p.x*25}" y1="${12+p.y*22}" x2="${14+q.x*25}" y2="${12+q.y*22}"/>`}));atlas.innerHTML=`<svg viewBox="0 0 220 208" aria-hidden="true">${lines.join('')}</svg>`+places.map(p=>`<button class="tour-node${tourPlaceForMap(game.save.map)?.id===p.id?' current':''}" data-place="${p.id}" style="left:${(14+p.x*25)/220*100}%;top:${(12+p.y*22)/208*100}%" aria-label="${p.name}로 이동" title="${p.name}"><i></i><span>${p.name.replace('시티','').replace('마을','')}</span></button>`).join('');section.querySelectorAll<HTMLButtonElement>('[data-region]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.region===region)));previousJournal='';refreshJournal();}
