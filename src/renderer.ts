...
import { experienceParticipants } from './battle';
...
import { SPECIES, STARTERS } from './pokemon';
...
export class Renderer {
  ...
  world(){const c=this.ctx,g=this.game,map=g.map;if(g.showingGymReward){this.gymRewardTop(c);if(g.dialogue)this.dialogue(c,false);return}if(g.showingCatch){this.catchTop(c);if(g.dialogue)this.dialogue(c,false);return}if(g.presentedBattle){this.battleTop(c);if(g.dialogue)this.dialogue(c);return}this.rect(c,0,0,W,H,'#101b20');const pos=g.position;
    ...
    if(map.id==='tour_eterna_forest'||map.id==='tour_coronet')for(const r of map.terrain??[])for(let y=r.y;y<r.y+r.h;y++)for(let x=r.x;x<r.x+r.w;x++)paintTallGrass(c,x*16,y*16,false,g.clock,this.images['grass-reference']);
    ...
    {const x=Math.round(pos.x),y=Math.round(pos.y);if(map.terrain?.some(r=>x>=r.x&&x<r.x+r.w&&y>=r.y&&y<r.y+r.h))paintTallGrass(c,x*16,y*16,true,g.clock*2+x,this.images['grass-reference']);}
    c.restore();
    if(g.labelTime>0){const alpha=Math.min(1,g.labelTime*2);c.globalAlpha=alpha;this.frame(c,5,5,Math.min(180,28+map.name.length*10),24);this.text(c,map.name,15,12,INK,9);c.globalAlpha=1}
  ...
  typePills(c:CanvasRenderingContext2D,types:string[],x:number,y:number){for(const [i,type]of types.entries()){this.rect(c,x+i*33,y,29,12,({'풀':'#6c9c4d','독':'#a16ca0','물':'#5c91b1','불꽃':'#c87d55','전기':'#bba149','노말':'#9b9983'} as Record<string,string>)[type]);this.text(c,type,x+14+i*33,y+2,'#fffdec',8,'center')}}
  ...
  lower(){const c=this.touch,g=this.game;this.hits=[];this.rect(c,0,0,W,H,'#e5e9d8');if(g.showingGymReward){this.gymRewardLower(c)}else if(g.showingCatch){this.catchLower(c)}else if(g.presentedBattle){this.battleLower(c)}else if(g.recoveryPreview&&g.dialogue){this.recovery(c)}else if(g.gymPreview&&g.dialogue){this.gymPreparation(c)}else if(g.panel==='field'||g.panel==='menu'){this.poketch(c);if(g.panel==='menu')this.menuLower(c)}else if(g.panel==='starters')this.starters(c);else if(g.panel==='party'||g.panel==='fieldHeal')this.party(c);else if(g.panel==='summary')this.moves(c);else if(g.panel==='bag')this.bag(c);else if(g.panel==='trainer')this.trainer(c);else if(g.panel==='options')this.options(c);
    if(g.dialogue){this.hits=[];const d=g.dialogue;if(d.choices&&d.page===d.pages.length-1&&d.shown>=d.pages[d.page].length){this.rect(c,5,117,246,71,'#e5e9d8');d.choices.forEach((choice,i)=>this.button(c,12,123+i*29,232,25,choice.label,()=>{d.selected=i;g.confirm()},i===d.selected))}else {this.frame(c,8,151,240,33);this.text(c,'Z / A  다음 이야기',128,162,INK,9,'center');this.hits.push({x:0,y:0,w:256,h:192,action:()=>g.confirm()})}}
  }
  ...
  gymRewardLower(c:CanvasRenderingContext2D){
    const guide=adventureGuide(this.game.save);this.topbar(c,'다음 모험을 준비하자');
    if(guide){this.text(c,guide.objective.title,128,40,INK,12,'center');this.text(c,guide.lines.join('\n'),128,63,'#617878',9,'center');}
    this.text(c,'목표 안내를 누르면 지도에 길을 표시해요.',128,99,INK,8,'center');
  }
  catchTop(c:CanvasRenderingContext2D){
    const g=this.game,p=g.save.party[g.caughtPreview!],species=SPECIES[p.species];
  ...
  catchLower(c:CanvasRenderingContext2D){
    const g=this.game,p=g.save.party[g.caughtPreview!];this.topbar(c,'포획 성공 · 파티 등록');
    this.text(c,`파티 ${g.caughtPreview!+1}번째 · ${g.save.party.length}/6마리`,128,38,INK,10,'center');
    ...
    this.text(c,'정보 보기에서 회복 · 선두 편성',128,99,'#617878',8,'center');
  }
  ...
  poketch(c:CanvasRenderingContext2D){const g=this.game;if(g.fieldMap){this.exploreMap(c);return;}this.rect(c,0,0,256,192,'#b95f4c');this.rect(c,4,0,248,192,'#da8265');this.rect(c,10,0,231,192,'#ebaa78');this.rect(c,14,7,219,177,'#664f48');this.rect(c,17,10,213,171,'#afbb88');for(let y=12;y<180;y+=3)this.rect(c,18,y,211,1,'#aab783');this.rect(c,24,17,200,24,'#89966b');this.text(c,'POKéTCH',31,24,'#3e543c',8);this.text(c,'01',216,24,'#3e543c',8,'right');
    const guide=adventureGuide(g.save);
    this.text(c,g.map.name,124,48,'#435d40',9,'center');
    if(guide){this.text(c,guide.objective.title,124,71,'#3e543c',12,'center');this.text(c,guide.lines.join('\n'),124,96,'#435d40',8,'center');}
    this.line(c,34,128,214,128,'#839569');
    if(g.save.party.length)this.pokemon(c,g.save.party[0].species,34,132,.4);else this.ball(c,50,148,9);
    this.text(c,`출전 가능 ${g.save.party.filter(p=>p.hp>0).length}/${g.save.party.length}`,145,136,'#435d40',8,'center');
    this.text(c,`배지 ${g.save.badges.length}/4 · 상처약 ${g.save.inventory.potions}`,145,151,'#435d40',8,'center');
    ...
    if(!g.dialogue&&g.panel==='field'){this.text(c,g.interactionHint??'X  메뉴',123,172,'#566e48',7,'center');this.hits.push({x:17,y:10,w:213,h:171,action:()=>g.cancel()});if(g.interactionHint)this.hits.push({x:17,y:163,w:213,h:18,action:()=>g.confirm()})}
  ...
  bag(c:CanvasRenderingContext2D){
    ...
    this.text(c,machines.length?'기술머신은 현재 보유만 가능합니다.':'기술머신 없음',14,109,'#78877c',7);
  ...
  trainer(c:CanvasRenderingContext2D){const s=this.game.save;this.topbar(c,'트레이너 카드');this.frame(c,10,32,236,130,'#e8e5c6');this.text(c,'빛나래 · 포켓몬 '+s.party.length+'마리',23,43,INK,10);GYMS.forEach((g,i)=>this.text(c,(s.badges.includes(g.badge)?'◆ ':'◇ ')+g.label,23+(i%2)*111,67+Math.floor(i/2)*20,INK,9));this.text(c,'소지금 '+s.money.toLocaleString()+'원',23,110);this.text(c,s.flags.ferryPass?'조사선 왕복 승선권':s.flags.researchDelivered?'운하항에서 승선 안내':s.flags.observationCollected?'장막 자료 → 축복 안내원':s.badges.length===4?'장막 관측 연구원을 만나자':'배지를 모아 다음 도시로',23,140,INK,8);this.back(c)}
  ...
  battleSelectionTop(c:CanvasRenderingContext2D){
    ...
    this.frame(c,7,33,242,94,'#f8f3da');this.pokemon(c,p.species,10,34,.95);
    ...
    this.text(c,species.moves.join(' · '),128,115,'#617878',8,'center');
    this.frame(c,3,132,250,57);this.text(c,battleHint(g.save,b).join('\n'),15,146,INK,9);
  }
  battleTop(c:CanvasRenderingContext2D){const b=this.game.presentedBattle!,view=this.game.battleFrame,p=view?.player??this.game.save.party[b.active],enemy=view?.enemy??b.enemy,attackDrop=view?.enemyAttackDrop??b.enemyAttackDrop,defenseDrop=view?.enemyDefenseDrop??b.enemyDefenseDrop;
    if(!this.game.dialogue&&(b.menu==='party'||b.menu==='heal')&&this.game.save.party[b.selected]){this.battleSelectionTop(c);return;}
    this.rect(c,0,0,256,192,'#e7edd5');for(let y=0;y<62;y+=3)this.rect(c,0,y,256,1,'#dce7cb');
    this.rect(c,0,62,256,130,'#c7d7aa');
    for(const [x,y,rx,ry]of [[196,84,53,12],[59,133,70,19]]){c.fillStyle='#a7bf86';c.beginPath();c.ellipse(x,y,rx,ry,0,0,Math.PI*2);c.fill();c.fillStyle='#d9e2b5';c.beginPath();c.ellipse(x,y-3,rx-3,ry-3,0,0,Math.PI*2);c.fill();}
    const effect=this.game.battleEffect,capture=this.game.captureMotion;
    if(enemy.hp>0&&!capture?.hideEnemy)this.pokemon(c,enemy.species,155+(effect?.target==='enemy'?effect.recoil:0),9);
    if(p.hp>0)c.drawImage(this.images['pokemon-back-'+p.species],20+(effect?.target==='player'?effect.recoil:0),65,96,96);
  ...
  battleGrowth(c:CanvasRenderingContext2D){
    ...
    this.text(c,levelUp?`Lv.${growth.before.level} → Lv.${p.level}`:`Lv.${p.level}  EXP +${growth.amount}`,89,63,levelUp?'#a36536':INK,10);
    this.text(c,`HP ${p.hp} / ${p.maxHp}`,89,82,INK,9);this.hp(c,89,97,147,p);
    this.text(c,levelUp?`최대 HP +${p.maxHp-growth.before.maxHp}`:'함께 싸워 얻은 경험치',16,115,'#617878',8);
  ...
  battleLower(c:CanvasRenderingContext2D){const g=this.game,b=g.presentedBattle!,p=g.save.party[b.active],view=g.battleFrame;
    ...
    if(view){this.topbar(c,b.kind==='gym'?`관장 ${gymById(b.gymId).name} · ${view.enemyIndex+1}/3`:'야생 비버니');this.frame(c,12,42,232,91,'#f8f3da');this.text(c,view.capture?'포획 시도 중':'전투 진행 중',128,56,INK,11,'center');this.text(c,`${SPECIES[view.player.species].name} · ${SPECIES[view.enemy.species].name}`,128,82,INK,10,'center');this.text(c,view.capture?'Z / A로 포획 결과를 확인하세요':'대사가 끝나면 행동을 선택하세요',128,111,'#617878',8,'center');return;}
    this.topbar(c,b.kind==='gym'?`관장 ${gymById(b.gymId).name} · ${b.enemyIndex+1}/3`:'야생 비버니');
    if(b.menu==='between'){
    ...
    if(b.menu==='party'||b.menu==='heal'){
      ...
      this.button(c,8,169,240,20,b.forcedSwitch?(b.kind==='gym'?'도전 중단':'도망친다'):'X  돌아가기',()=>b.forcedSwitch?g.requestBattleExit():g.cancel());return;
    }
    this.text(c,`${SPECIES[p.species].name}의 행동을 선택하세요`,128,36,INK,9,'center');
    const labels=b.menu==='moves'?SPECIES[p.species].moves:b.menu==='bag'?[`몬스터볼 (${g.save.inventory.pokeBalls})`,`상처약 (${g.save.inventory.potions})`]:['싸운다','가방','포켓몬',b.kind==='gym'?'도전 중단':'도망친다'];
    labels.forEach((label,i)=>this.button(c,8+(i%2)*124,54+Math.floor(i/2)*47,116,40,label,()=>{b.selected=i;g.selectBattle()},i===b.selected));
    if(b.menu==='actions')this.text(c,'경험치 참여 '+experienceParticipants(g.save,b).length+'마리',128,149,'#657b72',8,'center');
    if(b.menu==='actions')this.text(c,'방향키로 선택 · Z로 확인',128,165,INK,9,'center');
    else {
      this.frame(c,8,102,240,53,'#fbf9e6');
