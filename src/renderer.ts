import { getNexusStarterSpriteUrl } from './nexus-starter-art';
import { isNexusCampaign,speciesNumberLabel } from './nexus-starters';
import { trainerName,NEXUS_OPENING } from './nexus-opening-state';
...
export class Renderer {
  ...
  async load(){const names=['pokemon-shiny-130','pokemon-back-shiny-130',...FIELD_POKEMON.map(n=>'field-'+n),'center-reference','pokecenter_nurse','jubilife-reference','eevee-play','bedroom-reference','home-reference','town-reference','lab-reference','sandgem-reference','grass-reference','player_m','mom','prof_rowan','scientist_m','youngster','lass','old_man','middle_aged_man','ace_trainer_m','ace_trainer_f','scientist_f','pokemon_breeder_f','worker','rancher','school_kid_m','school_kid_f',...Object.keys(SPECIES).flatMap(n=>['pokemon-'+n,'pokemon-back-'+n])];await Promise.all(names.map(name=>new Promise<void>((resolve,reject)=>{const im=new Image();im.onload=()=>{this.images[name]=im;resolve()};im.onerror=()=>reject(Error('리소스를 불러오지 못했습니다: '+name));const custom=/^pokemon-(back-)?(\d+)$/.exec(name);im.src=(custom&&getNexusStarterSpriteUrl(Number(custom[2]),!!custom[1]))||'/assets/'+name+'.png'})));await document.fonts.load('10px Galmuri');
    ...
    this.images.heroCoral=recolorSprite(this.images.ace_trainer_m as HTMLImageElement,{'637b4a':'ad636b','425239':'693c52','8ca563':'df9a91','9c5a63':'b57947','de8c73':'ebba73','4a3139':'493e38'});
  ...
  trainer(c:CanvasRenderingContext2D){const s=this.game.save;this.topbar(c,'트레이너 카드');this.frame(c,10,32,236,130,'#e8e5c6');this.text(c,trainerName(s),23,43,INK,10);this.text(c,'포켓몬 '+s.party.length+'마리',232,44,INK,8,'right');GYMS.forEach((g,i)=>this.text(c,(s.badges.includes(g.badge)?'◆ ':'◇ ')+g.label,23+(i%2)*111,67+Math.floor(i/2)*20,INK,9));this.text(c,'소지금 '+s.money.toLocaleString()+'원',23,110);if(s.flags[NEXUS_OPENING.postcards])this.text(c,'빈 엽서 4장',232,111,INK,8,'right');this.text(c,s.flags.ferryPass?'조사선 왕복 승선권':s.flags.researchDelivered?'운하항에서 승선 안내':s.flags.observationCollected?'장막 자료 → 축복 안내원':s.badges.length===4?'장막 관측 연구원을 만나자':'배지를 모아 다음 도시로',23,140,INK,8);this.button(c,8,164,142,23,'Z  포켓몬도감',()=>showPokedex(this.game));this.back(c)}
