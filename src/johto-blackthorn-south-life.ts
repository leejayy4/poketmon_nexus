...
export function handleJohtoBlackthornSouthLife(g:Engine,event:string):boolean{
  ...
  if(event==='journeyWalker'){
    ...
    g.say('검은먹 남쪽 산길 여행자',[party,local,wins,route,source===JOHTO_ROUTE_29?'29번도로 북서쪽 선택 풀언덕에는 구구·꼬렛이 살고 동서 본선은 안전하다. 서쪽 무궁에서 회복·보급하고 북쪽46번도로로 돌아갈 수 있다.':'긴 풀은 야생 포켓몬의 생활 구역이고 가운데 본선과 동쪽 귀환 오르막은 안전하다.'],undefined,choices);return true;
  }
  ...
  g.say(authored?.name??titles[event],[...(authored?.pages??[]),party,local,wins,boundary]);return true;
}
