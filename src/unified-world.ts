...
export function createUnifiedWorld(base:Record<MapId,GameMap>):Partial<Record<MapId,GameMap>>{
  ...
  const route203Return=r203.warps.find(w=>w.to==='tour_jubilife')!;route203Return.spawn={x:37,y:24};
  jubilife.npcs.push({id:'jubilifeSouthGreeter',name:'축복 남문 여행자',sprite:'ace_trainer_f',x:18,y:32,facing:'left',dialogue:'jubilifeSouthGreeter'});
  jubilife.npcs.push({id:'jubilifeEastGuide',name:'축복 동문 길 안내원',sprite:'rancher',x:34,y:22,facing:'right',dialogue:'jubilifeEastGuide'});
  const oreburghGateDoor=oreburgh.warps.find(w=>w.to==='tour_oreburgh_gate_1f')!;
  const arrivalSpot=[[1,0],[-1,0],[0,1],[0,-1],[2,0],[-2,0]].map(([dx,dy])=>({x:oreburghGateDoor.x+dx,y:oreburghGateDoor.y+dy})).find(({x,y})=>oreburgh.walkable[y]?.[x]==='.'&&!oreburgh.warps.some(w=>w.x===x&&w.y===y)&&!oreburgh.npcs.some(n=>n.x===x&&n.y===y))!;
  oreburgh.npcs.push({id:'oreburghWestArrivalGuide',name:'무쇠 서문 동굴 안내원',sprite:'worker',...arrivalSpot,facing:'left',dialogue:'oreburghWestArrivalGuide'});
