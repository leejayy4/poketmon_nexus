...
import type { Choice, Dialogue, Direction, Panel, SaveData, Point, MapId } from './types';
...
export class Engine {
  ...
  get showingCatch(){return this.caughtPreview!==null&&!!this.dialogue&&this.dialogue.page===this.dialogue.pages.length-1}
  ...
  update(dt:number){dt=Math.min(dt,.05);this.clock+=dt;this.save.seconds+=dt;this.labelTime=Math.max(0,this.labelTime-dt);this.toastTime=Math.max(0,this.toastTime-dt);this.bumpCooldown=Math.max(0,this.bumpCooldown-dt);
  ...
  event(id:string){if(id==='tourHost'&&isWorldCenter(this.save.map))id='nurse';if(TOUR_MAPS[this.save.map as TourId]&&(id.startsWith('tour')||TOUR_RESIDENTS[this.save.map]?.some(n=>n.dialogue===id))){const pokemon=TOUR_POKEMON[this.save.map];if(pokemon&&id===pokemon.dialogue){this.say(pokemon.name,pokemon.pages);return}const resident=TOUR_RESIDENTS[this.save.map]?.find(n=>n.dialogue===id);if(resident){this.say(resident.name,resident.pages);return}const outdoors=getWorldOutdoors(this.map);const outdoor=outdoors?.objects.find(o=>o.event===id)??outdoors?.signs.find(o=>o.event===id);if(outdoor){this.say(outdoor.name,outdoor.pages);return}const room=TOUR_INTERIORS[this.save.map];const object=room?.objects.find(o=>o.event===id);if(object){this.say(object.name,object.pages);return}if(room&&id==='tourHost'){this.say(this.map.npcs[0].name,room.greeting);return}const p=tourPlaceForMap(this.save.map);this.say(id==='tourHost'?'시설 안내원':'마을 안내',p?[id==='tourHouse'?'주민들이 사는 집입니다.\n센터와 주요 시설 안을 둘러볼 수 있어요.':p.concept+'\n이곳은 자유롭게 둘러볼 수 있어요.',...TOUR_NEIGHBORS(p.id).map(n=>{const q=placeById(n)!;return(q.region===p.region?'길을 따라 ':'지방 연결편: ')+q.name+'로 이동할 수 있어요.'})]:['새잎마을의 시작 구간이에요.\n서쪽 출구가 축복시티로 이어집니다.']);return}if(id==='mom'){const pages=motherConversation(this.save);if(this.save.party.some(p=>p.hp<p.maxHp)){this.healParty();this.persist();pages.push('조금 쉬었다 가렴.\n포켓몬들이 모두 건강해졌단다.')}this.say('엄마',pages);return}
    ...
    if(id==='nurse'){
      ...
      this.healParty();this.save.inventory.potions=Math.max(2,this.save.inventory.potions);this.persist();
      this.say('간호사',['포켓몬들이 모두 건강해졌어요!\n상처약도 2개까지 보충했어요.', '모험 중 쓰러지면 여기로 데려올게요.\n언제든 편히 쉬러 오세요.']);return
    }
    if(id==='gatekeeper'){this.departure();return}
    if(id==='routeGuide'){this.healParty();this.save.inventory.pokeBalls=Math.max(5,this.save.inventory.pokeBalls);this.save.inventory.potions=Math.max(2,this.save.inventory.potions);this.persist();this.say('길 안내원',['포켓몬들을 쉬게 해 줄게요.\n모두 건강해졌어요!', '몬스터볼 5개, 상처약 2개까지\n부족한 도구도 보충했어요.', '풀밭을 걸으면 비버니가 나와요.\nHP를 절반 이하로 줄여 잡아 보세요.', '힘들면 도망쳐도 괜찮아요.\n흙길을 따라 동쪽이 새잎마을이에요.']);return}
  ...
  receive(species:number){this.panel='field';if(!grantPokemon(this.save,species)){this.say('',['이미 함께하고 있는 친구입니다.']);return}this.audio.play('receive');this.persist();const name=SPECIES[species].name;this.say('',[`${name}와(과) 친구가 되었다!`.replace('와(과)',species===1?'와':species===25?'와':'와'),`${name}가 파티에 등록되었다!\nX → 포켓몬에서 확인할 수 있다.`],()=>{if(species===25)this.say('연구원',['정말 고마워! 서두르지 말고\n이 친구의 마음을 알아가 줘.']);else this.say('은솔박사',['이제 너도 포켓몬 트레이너로구나!\n함께 마을을 둘러보고 오렴.'])});}
  ...
  onFieldStep(){
    if(!['route_s01','tour_eterna_forest','tour_coronet'].includes(this.save.map)||!this.save.flags.departureCleared)return;
  ...
  requestBattleExit(){
    ...
    this.say('도전 중단',['중단하면 다음 도전은 처음부터야.\n얻은 경험치와 현재 HP는 유지돼.','이번 배지와 승리 보상은 받을 수 없어.\n도전을 중단할까?'],undefined,[
  ...
  showSupplyHint(item:keyof SaveData['inventory']){
    ...
    this.say(name,[`${name}이 없어요.\n보충: ${supply.title}`,`${item==='pokeBalls'?'몬스터볼 5개':'상처약 2개'}까지 무료로 채워 줘요.\n보충 장소까지 길을 안내할까요?`],undefined,[
  ...
  actBattle(action:BattleAction){const b=this.battle;if(!b||b.result||this.dialogue)return;
    ...
    if(turn.outcome==='won'&&b.kind==='gym'){
      ...
      if(awardGym(this.save,b.gymId)){
        ...
        this.dialogue!.selected=1;this.gymReward={id:b.gymId,money,page};this.battleFrames=turn.frames??null;return;
      }
    }
    if(turn.outcome==='caught'){
      const index=this.save.party.length-1,caught=this.save.party[index];
      const close=()=>{if(this.battle!==b)return false;this.caughtPreview=null;this.battleFrames=null;this.battle=null;this.grassSteps=0;return true;};
      this.persist();this.say('',turn.pages,undefined,[
        {label:'정보 보기',action:()=>{if(close()&&this.save.party[index]===caught){this.partyIndex=index;this.summaryActionIndex=0;this.panel='summary';}}},
        {label:'계속 모험하기',action:()=>{close();}}
      ]);
      this.dialogue!.selected=1;this.caughtPreview=index;this.battleFrames=turn.frames??null;return;
    }
