...
export function handleBlackthornLife(g:Engine,event:string):boolean{
  ...
  const healthy=save.party.filter(mon=>mon.hp>0),species=Number(save.flags.blackthornTrainingSpecies??0),partnerName=SPECIES[species]?.name;
  ...
  if(save.map==='tour_blackthorn'&&event==='tourGuide'){
    g.say('검은먹시티 안내원',[partyLine,localLine,progress,'서쪽 얼음샛길, 북쪽 용의굴·전승 사당, 남쪽45번도로→46번도로→29번도로 동쪽 합류부를 실제 왕복할 수 있다.'],undefined,[
  ...
  if(event==='tourBlackthornTrainingWater'){
    ...
    g.say('용 수행 물길 관리판',[localLine,'찬물의 흐름을 살필 건강한 동료를 고르자. 출신 장소와 무관하게 참여할 수 있다.'],undefined,[...healthy.map(mon=>({label:SPECIES[mon.species].name,action:()=>{
      ...
      save.flags.blackthornTrainingSpecies=mon.species;save.flags.blackthornWaterBreathing=true;
  ...
  if(event==='blackthornHallRockTable'){
    ...
    if(!mon){g.say('푸른 암반 결 비교대',[`${partnerName}와 시작한 기록이 있지만 현재 건강한 상태로 파티에 없다.`,'센터에서 회복하거나 PC로 편성한 뒤 이어갈 수 있다.']);return true;}
  ...
  if(event==='dragonsDenLakeRail'){
    ...
    if(!mon){g.say('지하 호수 관찰 난간',[`${partnerName}와 이어 온 기록이 있지만 현재 건강한 상태로 파티에 없다.`,'검은먹센터에서 회복하거나 PC로 편성한 뒤 다시 이어갈 수 있다.']);return true;}
  ...
  if(event==='dragonsDenIslandStone'){
    ...
    if(!mon){g.say('중앙 섬 용 문양석',[`${partnerName}와 시작한 동굴 기록이 있지만 현재 건강한 상태로 파티에 없다.`,'회복·편성 뒤 같은 기록을 이어갈 수 있다.']);return true;}
  ...
  if(event==='dragonsDenShrineAltar'){
    ...
    if(!mon){g.say('용 전승 사당',[`${partnerName}와 이어 온 기록이 있지만 현재 건강한 상태로 파티에 없다.`,'검은먹센터에서 회복·편성한 뒤 다시 올 수 있다.']);return true;}
  ...
  if(/^blackthornCenter/.test(event)){g.say('검은먹센터 산악 안내',[partyLine,localLine,progress,'실제 회복은 간호사, 편성은 PC를 이용하자. 서쪽 얼음샛길·북쪽 용의굴·남쪽45번도로는 양방향이며45번도로와46번도로의 턱은 별도 오르막으로 돌아온다.']);return true;}
  ...
  if(/^blackthornHome/.test(event)){g.say('검은먹 산악 생활 기록',[partyLine,localLine,progress,'주민과 포켓몬이 방한 천·찬물·절벽 바람 기록을 함께 관리한다. 조사만으로 회복이나 보상은 생기지 않는다.']);return true;}
  ...
  if(event==='tourBlackthornIcePathStone'||event==='tourBlackthornDragonDenBoundary'||event==='tourBlackthornRoute45Stone'||event==='tourBlackthornHallStone'){
    ...
    g.say('검은먹 방향 기록',[...pages,partyLine,progress]);return true;
  }
  return false;
}
