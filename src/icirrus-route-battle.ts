...
export function showIcirrusRouteBattle(g:Engine,start?:()=>void){
  ...
  const prepare=()=>{if(current()&&start)showTrainerPreparation(g,current,start);};
  const f=ROUTE_EIGHT_JOURNEY,partner=routeEightTrackedPartner(save),participated=save.flags[f.participated]===true;
  const result=participated?(partner?`${SPECIES[partner.species].name}가 실제로 상대를 쓰러뜨린 기록\n출발 Lv.${Number(save.flags[f.level]??partner.level)} → 현재 Lv.${partner.level} · HP ${partner.hp}/${partner.maxHp}`:`${SPECIES[Number(save.flags[f.partner]??0)]?.name??'8번도로 동료'}가 실제로 상대를 쓰러뜨렸지만 현재 파티에서 확인되지 않는다.`):won?'과거 승리는 있지만 8번도로 현지 동료의 실제 상대 격파 참가는 아직 확인되지 않았다.':'건강한 8번도로 딱정곤·쪼마리를 선두로 두면 실제 성장 참가를 기록할 수 있다.';
  g.say('8번도로 습지 트레이너',[
    ...
    ...(start?[{label:won?'현지 동료로 재확인전':'동료·기술을 고르고 도전',action:prepare}]:[]),
