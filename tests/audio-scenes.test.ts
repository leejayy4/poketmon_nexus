...
test('eight original scenes have different melodies and bounded gentle voices',t=>{
  const h=harness(t);h.audio.toggle();const c=h.contexts[0],signatures=[];
  for(const scene of ['town','route','cave','wild','gym','cinnabar','icirrus','dragonspiral'] as AudioScene[]){
