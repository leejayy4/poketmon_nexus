...
import { PLACES,TOUR_SPAWNS,TOUR_INTERIORS,type TourId } from '../src/explore-world';
...
test('all interiors first guide outside and recognize a destination already being visited',()=>{
  for(const id of Object.keys(TOUR_INTERIORS)){const g=tour(id),place=id.replace(/_(hall|center)$/,'') as TourId;assert.equal(planTourNavigation(g.save,place)?.status,'arrived');const target=place==='tour_jubilife'?'tour_humilau':'tour_jubilife';const r=planTourNavigation(g.save,target)!;assert.equal(r.status,'walking');assert.equal(r.maps[1],place);assert.equal(r.exit?.entry,'down');}
});
