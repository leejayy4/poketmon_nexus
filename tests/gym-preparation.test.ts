...
test('blocked challenges tell the actual unmet requirement and never offer a battle',()=>dom(()=>{
  const g=new Engine();g.save=newSave();g.panel='field';g.challengeGym('roark');assert.match(g.dialogue!.pages[0],/첫 파트너/);
...
test('cancel and touch preparation return preserve progression and dismiss the preview',()=>dom(()=>{
  const g=new Engine();g.save=ready();g.panel='field';const before=structuredClone(g.save);g.challengeGym('roark');assert.equal(g.gymPreview,'roark');g.cancel();assert.equal(g.dialogue,null);assert.equal(g.gymPreview,null);
...
test('preview draws the actual teams for every gym and confirms a low-level challenge with no upfront cost',()=>dom(()=>{
  for(let i=0;i<GYMS.length;i++){
    const g=new Engine();g.save=ready();g.panel='field';g.save.badges=GYMS.slice(0,i).map(g=>g.badge);const before=structuredClone(g.save),words:string[]=[];
