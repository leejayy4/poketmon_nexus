...
test('preview draws the actual teams for every gym and confirms a low-level challenge with no upfront cost',()=>dom(()=>{
  for(let i=0;i<GYMS.length;i++){
    ...
    r.click(128,136);assert.equal(g.battle!.gymId,GYMS[i].id);assert.equal(g.battle!.enemy.species,GYMS[i].team[0][0]);assert.equal(g.gymPreview,null);assert.deepEqual(g.save,before);
  }
}));
