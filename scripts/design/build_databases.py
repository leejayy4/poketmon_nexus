...
COMMON = ('작성일: 2026-09-06 · NEXUS 설계 DB v0.1 · **기획 데이터, 게임 미구현**\n\n'
...
def render_encounters():
    ...
    text += ('## 세부 맵으로 나눌 때의 주의\n\nS17의 세 호수, S18의 꽃밭·발전소·돌탑, K18·J13·U10의 도로 묶음은 각 하위 맵으로 분리해야 한다. '
             '이번 풀은 해당 생태 유형의 후보이며 모든 하위 지도에 같은 풀을 복사한다는 뜻이 아니다. `mapId/subArea`를 정하기 전까지 런타임 적용을 금지한다.\n\n'
...
def render_quests():
    ...
    text += ('\n## 2. 저장과 재진입\n\n'
             ...
             '대화 페이지나 콜백을 저장하지 않는다. 알·부화·진화·보관함 기능은 현재 미구현이므로 별도 저장 이행이 필요하다.\n\n'
...
def render_index():
    ...
    text += ('\n## 3. 데이터 파일과 수정 순서\n\n'
             ...
             '기존 종 ID·MapId·수령 플래그는 변경하지 않는다. 새 보관함·진화·레벨·배지·가방 데이터는 현재 저장 검사를 통과할 수 없으므로 '
