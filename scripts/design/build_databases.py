...
def render_dex():
    ...
    text += ('\n전국번호·한국어 이름·타입·종족값·특성은 [고정한 PokéAPI 종 자료](' + SOURCE_BASE + 'pokemon_species.csv) 및 연결 표를 사용했다. '
             ...
             '## 2. 표의 의미\n\n'
             '- `요청`은 앞선 123종, `추가`는 이번에 보완한 종이다. 현재 런타임 지원 범위는 [공통 데이터 기준](WORLD_DATA_STANDARDS.md)의 코드 대조표를 확인하며 이 설계표 전체와 구분한다.\n'
...
def render_index():
    ...
    text += ('운영 기준: [지방·도시 맵/스토리 구현](DEVELOPMENT.md#0-최우선-목표와-지방도시-작업-단위) · [도시 완료 후 QA](AGENTS.md#검증과-도시-완료). 설계 표의 CH·해금·테스트 이력으로 현행 우선순위와 검증 시점을 덮어쓰지 않는다.\n\n')
    text += ('공통 참조: [네 지방 데이터·세계관 기준](WORLD_DATA_STANDARDS.md) · [장소·도로·교통 명세](WORLD_ROUTES.md). '
             '현재 종·기술·진화·조우 수량과 실제 MapId 연결은 공통 기준의 코드 대조표를 따른다. '
             '게임 표시 번호는 지방별 기준 문서, 원작 번호는 지리 비교 자료로 구분한다.\n\n')
